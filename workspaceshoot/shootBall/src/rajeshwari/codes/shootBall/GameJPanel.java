...
public  class  GameJPanel extends JPanel {
	...
	public void start(double velocityMagnitude, double angleInRadian) {
		ball.setVelocity(velocityMagnitude, angleInRadian);
	}
...
//			} // block left side overlapping check
	...
	private boolean isOverlapped(Blocks block) {
		double ballCentreX = ball.getX() + Ball.WIDTH / 2;
		double ballCentreY= ball.getY() + Ball.HEIGHT /2;
		for(double y=block.getY(); y<=block.getY() + BLOCK_HEIGHT; y++ ){
			if (distanceBetweenTwoPoints(block.getX()+ BLOCK_WIDTH, ballCentreX, y, ballCentreY) < Ball.WIDTH / 2){
				return true;
		     }
			if (distanceBetweenTwoPoints(block.getX(), ballCentreX, y, ballCentreY) < Ball.WIDTH / 2){
				return true;
			}
		}
		for( double x= block.getX(); x< block.getX()+ BLOCK_WIDTH; x++){
			if(distanceBetweenTwoPoints(x, ballCentreX, block.getY(), ballCentreY) < Ball.HEIGHT / 2){
				return true;
		     }
			if(distanceBetweenTwoPoints(x, ballCentreX, block.getY()+ BLOCK_HEIGHT, ballCentreY) < Ball.HEIGHT / 2){
				return true;
			}
		}
		return false;
